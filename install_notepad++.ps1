function Install {
    ...
    $DownloadUrl = "https://github.com/notepad-plus-plus/notepad-plus-plus/releases/download/v8.5.7/npp.8.5.7.portable.x64.7z"
    $KiloathDir = Join-Path $HOME "KiloathApp"
    $Directory = Join-Path $KiloathDir "Notepad++"
    $Target = Join-Path $Directory "npp.8.5.7.portable.x64.7z"
    $BinDir = "$Directory"
    $BinExe = "$BinDir\notepad++.exe"
    $AppName = "Notepad++"
    ...
    if(($file = Get-Item $Target -ErrorAction SilentlyContinue) -And ($file.Length -eq 3836284)) {
        Write-Host "你已下載最新版"
    }
    else {
